...
const initialState = {
  ...
  searchedTrending: ["alchol", "beer", "drinks"]
};
...
const onSearch = (state, action) => {
    const updatedState = {
        ...
        // trending: action.payload[0].trending
    }

    return updatedObject(state, updatedState);
};

const searchLoaded = (state, action) => {
    const updatedState = {
        // users: action.payload[0].users,
        // articles: action.payload[0].articles,
        // trending: action.payload[0].trending
...
const searchUnloaded = (state, action) => {
    return {}
};
...
export default (state = initialState, action) => {
  switch (action.type) {
    ...
    case SEARCH_LOADED:
      return searchLoaded(state, action);
    case SEARCH_UNLOADED:
      return searchUnloaded(state, action);
    default:
      return state;
  }
};
